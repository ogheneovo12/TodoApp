...
class App extends React.Component {
  ...
  onInputChange(e) {
    ...
    this.setState(prevState => {
      const filteredTodos = prevState.TodosApi.filter(todo =>
        todo.name.toLowerCase().includes(prevState.text.toLowerCase())
      );
      return {
  ...
  addTodo() {
    if (this.state.text !== "") {
      ...
      const NewTodos = [
        { name, done: false, id: this.generateId() },
        ...this.state.TodosApi
      ];
      ...
      <div>
        <div className="container">
          ...
          <Todos
            ...
            filtered={this.state.filteredTodos}
            remTodo={this.remTodo}
            TodosApi={this.state.TodosApi}
          />
        </div>
      </div>
    );
